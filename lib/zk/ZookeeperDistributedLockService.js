...
module.exports = class ZookeeperDistributedLockService extends DistributedLockService {
    ...
    lock(id = `lock-${uuid()}`, timeout = 360, cb = () => {}) {
        ...
        let reentrantLock = new config.ReentrantLock(this[Symbol.for('client')], config, id, timeout);
        co(function *() {
            yield reentrantLock.lock();
        }.bind(this)).then(() => {
            defer.resolve(reentrantLock);
            typeof cb === 'function' && Reflect.apply(cb, null, [reentrantLock]);
        }).catch(err => {
            defer.reject(err);
