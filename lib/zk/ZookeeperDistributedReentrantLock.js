...
module.exports = class ZookeeperDistributedReentrantLock extends DistributedReentrantLock {
    lock() {
        ...
        this.client.exists(lockPath, null, (err, stat) => {
            if (err) {
                Log.error(`unable exists ${lockPath} failed`, err.stack);
                return this.fail();
            }
            ...
            if (stat) {
                ...
                this.client.create(lockPath, null, zookeeper.CreateMode.PERSISTENT, (err, path) => {
                    if (err) {
                        Log.error(`unable to create node: ${lockPath}`, err.stack);
                        this.fail();
                    }
                    Log.debug(`zk node key created: ${path}`);
                    this.lockQueue();
                });
            }
        });
    ...
    lockQueue() {
        let lockPath = `${this.config.area}/${this.id}`;
        this.client.create(`${lockPath}/lock`, null, zookeeper.CreateMode.EPHEMERAL_SEQUENTIAL, (err, path) => {
            if (err) {
                Log.error(`unable to create node: ${lockPath}/lock`, err.stack);
                this.fail();
            }
            Log.debug(`zk node lock created: ${path}`);
     ...
     */
    check() {
        this.client.getChildren(`${this.config.area}/${this.id}`, null, (err, children) => {
            if (err) {
    ...
    wait(last) {
        ...
        this.client.exists(`${this.config.area}/${this.id}/${last}`, watch, (err, stat) => {
            if (err) {
                Log.error(`unable check ${this.config.area}/${this.id}/${last} exists`, err.stack);
                return this.fail();
            }
