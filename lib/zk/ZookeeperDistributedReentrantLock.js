...
class ZookeeperDistributedReentrantLock extends DistributedReentrantLock {
    lock(expire) {
        let defer = Q.defer();
        this.defer = defer;
        this.expire = expire;
        co(function *() {
            ...
            logger.debug(`lock ${this.id} is exists ${!(!existsKey)}`);
            if (!existsKey) {
                logger.debug(`lock ${this.id} create key...`);
                try {
                    yield thunkify(this.client.create).apply(this.client, [lockPath, null]);
                } catch (err) {
                    if (err.getCode() === zookeeper.Exception.NODE_EXISTS) {
                        logger.warn(`lock ${this.id} create key exists.`);
                    } else {
                        throw err;
                    }
                }
    ...
    * lockQueue() {
        ...
        logger.debug(`lock ${this.id} resource for ${this.resource}`);
        yield this.check();
    }
    ...
    * unlock() {
        ...
        this.isUnlock = true;
        logger.info(`unlock ${this.id} success for ${this.locked}`);

        try {
            yield this.clearKey();
        } catch (err) {
            logger.warn(`clear key ${this.id} error`, err.stack);
        }
    }
    ...
    * cancel() {
        if (this.resource && !this.locked && !this.isCancel) {
            ...
            this.isCancel = true;
            logger.info(`lock ${this.id} await timeout, removed resource ${this.resource}`);
    ...
    * check() {
        if (this.isCancel) {
            logger.warn(`lock ${this.id} is canceled`);
            return;
        }
        let children = yield this.list(`${this.config.area}/${this.id}`);
        let sorted = children.sort();
        logger.debug(`lock ${this.id} waiting children ${util.inspect(sorted)}`);

        if (this.resource === sorted[0]) {
            this.locked = this.resource;
            this.lockTime = new Date();
            this.setExpireTimeout();
            logger.info(`lock ${this.id} success for ${this.locked}`);
            this.defer.resolve();
            return;
        }
    ...
    * wait(last) {
        logger.info(`lock ${this.id} resource ${this.resource} waiting for ${last}`);
        let watch = () => {
            if (this.isCancel) {
                logger.warn(`lock ${this.id} is canceled`);
                return;
            }
            logger.debug(`lock ${this.id} wait watch ${last} retrying...`);
        ...
        if (!stat) {
            // 上一个如果不存在则可能已经被释放了
            logger.debug(`lock ${this.id} wait ${last} not found. retry...`);
            yield this.check();
        }
    }
    ...
    * rollback() {
        ...
        logger.error(`lock failed, cleaning up existing sequential zk node: ${nodePath}`);
    ...
    setExpireTimeout() {
        ...
        this.expireTimeout = setTimeout(() => {
            if (!this.isCancle && !this.isUnlock) {
                logger.info(`lock ${this.id} resource ${this.locked} expire.`);
                this.unlockSync();
            }
        }, this.expire);
    }
}
module.exports = ZookeeperDistributedReentrantLock;
